...
class Writer {
  ...
  findAsset (assetPath) {
    ...
    const yarnModPath = this.path.join(process.cwd(), '.yarn', 'cache');
    const zipFile = this.glob.sync(`${yarnModPath}/${assetPath.split('/')[0]}-*`).find(file => this.fs.existsSync(this.path.join('/', file, 'node_modules', assetPath)));
    if (zipFile) {
      return this.path.join('/', zipFile, 'node_modules', assetPath);
    }
