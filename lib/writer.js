...
class Writer {
  ...
  createOutputFiles () {
    ...
    // edit the title so it shows the api description
    const indexHtml = this.fs.readFileSync(indexPath, 'utf8');
    const projectInfo = JSON.parse(this.api.project);
    const indexContent = indexHtml.toString()
      .replace(/__API_NAME__/, projectInfo.name);
    // create target file
    const finalIndexPath = this.path.join(this.opt.dest, 'index.html');
    this.fs.writeFileSync(finalIndexPath, indexContent);
