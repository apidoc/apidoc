...
class Writer {
  ...
  write () {
    if (this.opt.simulate) {
      this.log.warn('Simulation mode enabled: no files created.');
      return false;
    }
  ...
  createOutputFiles () {
    ...
    if (!this.opt.copyDefinitions) { this.fs.writeFileSync(apiDefinitionsJsPath, this.api.definitions ?? '??'); }

    const apiDefinitionsJsonPath = apiDefinitionsJsPath + 'on';
    if (!this.opt.copyDefinitions) { this.writeJsonFile(apiDefinitionsJsonPath, this.api.definitions ?? '??'); }
  }
