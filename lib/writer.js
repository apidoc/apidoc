...
class Writer {
  ...
  createOutputFiles () {
    ...
    if (this.opt.apidataFile) {
      this.log.verbose('Writing apidata.json to: ' + this.opt.apidataFile);
      this.fs.writeFileSync(this.path.join(this.opt.apidataFile, 'apidata.json'), this.api.data);
    }
