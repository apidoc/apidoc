...
function _getParentInfos (field) {
  let i = field.length;
  while (i--) {
    if (field.charAt(i) === '.') {
      const path = field.substring(0, i);
      const _parent = parents[path];
      if (_parent) { return { path: path, type: _parent }; }
    }
  }
}
...
function parse (content, source, defaultGroup) {
  ...
  const field = matches[6].replace(/\[/g, '.').replace(/\]/g, '');

  if (type && type.indexOf('Object') === 0) { parents[field] = type; }
  ...
  return {
    ...
    parentInfo: _getParentInfos(field),
    field: field,
