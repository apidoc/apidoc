...
function _getObject (field) {
  let i = field.length;
  while (i--) {
    if (field.charAt(i) === '.') {
      const object = field.substring(0, i);
      if (objects[object]) { return object; }
    }
  }
  return '';
}
...
function parse (content, source, defaultGroup) {
  ...
  if (type && type.indexOf('Object') === 0) { objects[field] = true; }
