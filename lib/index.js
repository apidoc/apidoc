/*
 * apidoc
 * https://apidocjs.com
 *
 * Copyright (c) 2013 inveris OHG
 * Author Peter Rottmann <rottmann@inveris.de>
 * Licensed under the MIT license.
 */
const fs = require('fs-extra');
const path = require('path');

const core = require('./core/index');
const defaults = require('./defaults');
const optionsProcessor = require('./options');
const PackageInfo = require('./package_info');

const app = {
  log: {},
  markdownParser: null,
  options: {},
};

// Display uncaught Exception.
process.on('uncaughtException', err => {
  console.error(new Date().toUTCString() + ' uncaughtException:', err.message);
  console.error(err.stack);
  process.exit(1);
});

/**
 * Create the documentation
 *
 * @param  {Object} options See `apidoc --help`
 * @returns {Mixed} true = ok, but nothing todo | false = error | Object with parsed data and project-informations.
 */
function createDoc (options) {
  // process options
  app.options = optionsProcessor.process(options);

  // get the default logger
  app.log = defaults.getLogger(app.options);

  // get the markdown parser
  app.markdownParser = defaults.getMarkdownParser(app.options);

  try {
    // generator information
    const pkgjson = require('../package.json');
    core.setGeneratorInfos({
      name: pkgjson.name,
      time: new Date(),
      url: pkgjson.homepage,
      version: pkgjson.version,
    });
    core.setLogger(app.log);
    core.setMarkdownParser(app.markdownParser);

    const packageInfo = new PackageInfo(app);
    core.setPackageInfos(packageInfo.get());

    // this is holding our results from parsing the source code
    const api = core.parse(app.options);

    if (api === true) {
      app.log.info('Nothing to do.');
      return true;
    }
    if (api === false) { return false; }

    if (app.options.simulate) {
      app.log.warn('Simulation mode enabled: no files created.');
    } else {
      if (app.options.single) {
        createSingleFile(api);
      } else {
        createOutputFiles(api);
      }
    }

    if (app.options.verbose) {
      app.log.info('Done.');
    }

    return api;
  } catch (e) {
    app.log.error(e.message);
    if (e.stack) { app.log.debug(e.stack); }
    return false;
  }
}

/**
 * Save parsed data to files
 *
 * @param {Object[]} blocks
 * @param {Object} packageInfos
 */
function createOutputFiles (api) {
  createDir(app.options.dest);

  app.log.verbose('copying template ' + app.options.template + ' to: ' + app.options.dest);
  fs.copySync(app.options.template, app.options.dest);

  // copy vendored libraries
  const vendorPath = path.join(app.options.dest + 'vendor');

  // requirejs
  app.log.verbose('copying require library to: ' + vendorPath);
  fs.copySync('./node_modules/requirejs/require.js', path.join(vendorPath, 'require.js'));

  // jquery
  app.log.verbose('copying jquery library to: ' + vendorPath);
  fs.copySync('./node_modules/jquery/dist/jquery.min.js', path.join(vendorPath, 'jquery.min.js'));

  // handlebars
  app.log.verbose('copying handlebars library to: ' + vendorPath);
  fs.copySync('./node_modules/handlebars/dist/handlebars.min.js', path.join(vendorPath, 'handlebars.min.js'));

  // bootstrap
  app.log.verbose('copying bootstrap js library to: ' + vendorPath);
  fs.copySync('./node_modules/bootstrap/dist/js/bootstrap.min.js', path.join(vendorPath, 'bootstrap.min.js'));
  app.log.verbose('copying bootstrap css to: ' + vendorPath);
  fs.copySync('./node_modules/bootstrap/dist/css/bootstrap.min.css', path.join(vendorPath, 'bootstrap.min.css'));

  // webfontloader
  app.log.verbose('copying webfontloader library to: ' + vendorPath);
  fs.copySync('./node_modules/webfontloader/webfontloader.js', path.join(vendorPath, 'webfontloader.js'));

  // Write api_data
  const apiDataJsPath = path.join(app.options.dest, 'api_data.js');
  writeJSFile(apiDataJsPath, '{ "api": ' + api.data + ' }');

  const apiDataJsonPath = apiDataJsPath + 'on';
  writeJsonFile(apiDataJsonPath, api.data);

  // Write api_project
  const apiProjectJsPath = path.join(app.options.dest, 'api_project.js');
  writeJSFile(apiProjectJsPath, api.project);

  const apiProjectJsonPath = apiProjectJsPath + 'on';
  writeJsonFile(apiProjectJsonPath, api.project);

  // Write api_definitions
  const apiDefinitionsJsPath = path.join(app.options.dest, 'api_definition.js');
  if (!app.options.copyDefinitions) { fs.writeFileSync(apiDefinitionsJsPath, api.definitions ?? '??'); }

  const apiDefinitionsJsonPath = apiDefinitionsJsPath + 'on';
  if (!app.options.copyDefinitions) { writeJsonFile(apiDefinitionsJsonPath, api.definitions ?? '??'); }
}

/**
 * Write a JSON file
 *
 * @param {string} dest Destination path
 * @param {string} data Content of the file
 */
function writeJsonFile (dest, data) {
  app.log.verbose(`writing json file: ${dest}`);
  fs.writeFileSync(dest, data + '\n');
}

/**
 * Write js file
 *
 * @param {string} dest
 * @param {string} data
 */
function writeJSFile (dest, data) {
  app.log.verbose(`writing js file: ${dest}`);
  switch (app.options.mode) {
    case 'amd':
    case 'es':
      fs.writeFileSync(dest, 'export default ' + data + ';\n');
      break;
    case 'commonJS':
      fs.writeFileSync(dest, 'module.exports = ' + data + ';\n');
      break;
    default:
      fs.writeFileSync(dest, 'define(' + data + ');' + '\n');
  }
}

/**
 * Create a directory
 *
 * @param {string} dir Path of the directory to create
 */
function createDir (dir) {
  app.log.verbose('creating dir: ' + dir);
  fs.mkdirsSync(dir);
}

function createSingleFile (api) {
  // dest is a file path, so get the folder with dirname
  createDir(path.dirname(app.options.dest));

  // create target file
  app.log.verbose('generating file: ' + app.options.dest);
  fs.writeFileSync(app.options.dest, fs
    .readFileSync(app.options.template)
    .toString()
    .replace('__API_DATA__', api.data)
    .replace('__API_PROJECT__', api.project),
  );
}

module.exports = {
  createDoc: createDoc,
};
