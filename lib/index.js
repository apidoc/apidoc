...
function createDoc (options) {
  ...
  // add a trailing slash to output destination
  ...
  try {
    ...
    if (pkgInfo.output && app.options.dest === defaults.DEFAULT_DEST) {
      app.log.debug('config file is overriding default output path');
      // keep a trailing slash
      app.options.dest = path.normalize(pkgInfo.output) + path.sep;
    }
    // do the same for input
    if (pkgInfo.input instanceof Array && app.options.src[0] === defaults.DEFAULT_SRC[0]) {
      app.log.debug('config file is overriding default input path');
      // keep a trailing slash
      const input = pkgInfo.input.map(p => path.resolve(p) + path.sep);
      app.options.src = input;
    }
