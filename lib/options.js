...
function process (options) {
  ...
  // if a apidata json directory is given, check it
  if (options.apidataFile) {
    console.log('here');
    // make sure that we are provided a directory
    if (!fs.statSync(options.apidataFile).isDirectory()) {
      throw new Error('[error] Invalid option: --apidataFile must be a path to a file. Directory provided.');
    }
  }
