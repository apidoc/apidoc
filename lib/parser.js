...
Parser.prototype.parseFile = function(filename)
{
    ...
    self.elements = self.blocks.map(function(block, i){
        app.log.debug("count elements in block " + i + ": " + block.length);
        ...
        for(var j = 0; j < elements.length; j += 1)
        {
            ...
            if(self.parsers[element.name])
            {
                ...
                catch(e)
                {
                    ...
                    throw new ParserError('Uncatched error.', self.filename, (blockIndex + 1), element.sourceName, element.source);
                }
